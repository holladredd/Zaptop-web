...
import { AiOutlineShareAlt, AiOutlineCheckCircle, AiOutlineArrowLeft, AiOutlineCopy, AiOutlineTwitter, AiOutlineFacebook } from 'react-icons/ai';
import Swal from 'sweetalert2';

const socialTasks = [
  { id: 1, title: 'Refer a Friend', description: 'Invite friends using your unique referral code', points: 500, action: 'Copy Link', icon: AiOutlineCopy },
  { id: 2, title: 'Share on Twitter', description: 'Tweet about ZapTop to your followers', points: 200, action: 'Share', icon: AiOutlineTwitter },
  { id: 3, title: 'Share on Facebook', description: 'Post about ZapTop on your timeline', points: 200, action: 'Share', icon: AiOutlineFacebook },
  { id: 4, title: 'Join Community', description: 'Join our Telegram community group', points: 300, action: 'Join', icon: AiOutlineShareAlt },
];
...
export default function SocialTasks() {
  ...
  const handleTask = async (task) => {
    if (task.id === 1) {
      await navigator.clipboard.writeText(`Join ZapTop with my code: ${referralCode}`);
      Swal.fire({
        icon: 'success',
        title: 'Copied!',
      ...
      <Layout title="Social Tasks">
        ...
        >
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-2xl font-bold text-slate-800">Social Tasks</h2>
              <p className="text-slate-500">Share and engage to earn points</p>
            </div>
          </div>

          {/* Referral Code Card */}
          <motion.div variants={itemVariants} className="bg-gradient-to-r from-pink-500 to-rose-500 rounded-xl p-6 text-white">
            <h3 className="text-lg font-semibold mb-2">Your Referral Code</h3>
            <p className="text-pink-100 text-sm mb-4">Share this code with friends and earn 500 points per referral</p>
                ...
                >
                  <div className="flex items-center gap-4">
                    <div className="w-14 h-14 bg-pink-100 rounded-xl flex items-center justify-center">
                      <task.icon className="w-7 h-7 text-pink-600" />
                    </div>
                    ...
                    <div className="text-right">
                      <p className="text-pink-600 font-bold mb-2">+{task.points}</p>
                      {!isCompleted ? (
                        <button
